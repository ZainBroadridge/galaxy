...
import {
  AUTHENTICITY_CLAIM, VOTE_EVENT_ABI, hashEventMetadata, packProposalConfig,
} from '@pv/shared';
...
export async function createEvent(wallet, input) {
  ...
  const used = await query(
    `SELECT count(*)::int AS count FROM events WHERE creator_address=$1 AND created_at >= now()-interval '24 hours'`,
    [creator],
  );
  ...
  const proposalConfig = packProposalConfig(metadata.proposals.map((item) => item.options.length));
  ...
  const created = await transaction(async (client) => {
    const result = await client.query(
      ...
      [creator, token.tokenAddress, token.name, token.symbol, token.decimals, metadata.title, metadata.description,
        JSON.stringify(metadata.proposals), hash, proposalConfig.toString(), input.recordDateAt, input.tokenToVoteRatio,
        voteUnit.toString(), input.votingStartAt, input.votingEndAt, input.discoveryMode, authenticityStatus, input.snapDeliveryMode],
    );
    const event = result.rows[0];
    const job = await enqueueJob({
      eventId: event.id, type: 'BUILD_SNAPSHOT', dedupeKey: `snapshot:${event.id}`,
      message: 'Snapshot queued', client,
    });
    return { event, job };
  });
  kickJobRunner();
  return { event: serializeEvent(created.event), job: serializeJob(created.job) };
}
...
export async function eventView(id, walletInput) {
  ...
  if (wallet && row.wallet_snapshot_balance !== null && (!vote || vote.status === 'FAILED') && row.contract_address && row.deployment_block !== null) {
    onChainVoted = await new Contract(row.contract_address, VOTE_EVENT_ABI, provider).hasVoted(wallet).catch(() => false);
  }
...
export async function votingDashboard(walletInput) {
  ...
  const rows = await query(
    `SELECT e.*,se.raw_balance,se.voting_power,v.status AS vote_status,v.transaction_hash
     FROM snapshot_entries se JOIN events e ON e.id=se.event_id
     LEFT JOIN votes v ON v.event_id=e.id AND v.voter_address=se.wallet_address
     LEFT JOIN snap_subscriptions s ON s.wallet_address=se.wallet_address AND s.token_address=e.token_address AND s.enabled=true
     WHERE se.wallet_address=$1 AND e.deployment_block IS NOT NULL AND e.voting_end_at>now() AND e.status<>'FAILED'
       AND e.discovery_mode<>'DIRECT_LINK'
       AND (e.discovery_mode='PUBLIC_ELIGIBLE' OR (e.discovery_mode='SUBSCRIBERS_ONLY' AND s.wallet_address IS NOT NULL))
     ORDER BY e.voting_end_at,e.created_at DESC`,
    [wallet],
  );
  return rows.rows.map((row) => serializeEvent(row, {
    eligibility: { eligible: true, snapshotBalance: String(row.raw_balance), votingPower: String(row.voting_power), hasVoted: Boolean(row.vote_status && row.vote_status !== 'FAILED') },
...
export async function organiserDashboard(wallet) {
  const rows = await query('SELECT * FROM events WHERE creator_address=$1 ORDER BY created_at DESC', [normalizeAddress(wallet)]);
  return rows.rows.map((row) => serializeEvent(row));
}
...
export async function eventResults(id, walletInput) {
  ...
  for (let index = 0; index < event.proposals.length; index += 1) {
    // Keep the explicit results read gentle on free-tier RPC throughput.
    tallies.push((await contract.getProposalTallies(index)).map((value) => value.toString()));
  }
  return { event: serializeEvent(event), proposals: event.proposals.map((proposal, index) => ({ ...proposal, tallies: tallies[index] })) };
}

export async function retryEvent(id, wallet) {
  const event = await getEventRow(id);
  if (event.creator_address !== normalizeAddress(wallet)) throw new HttpError(403, 'Only the event creator can retry it.', 'FORBIDDEN');
  let type; let dedupeKey; let message;
  if (!event.snapshot_root) {
    type = 'BUILD_SNAPSHOT'; dedupeKey = `snapshot:${id}`; message = 'Snapshot retry queued';
    await query("UPDATE events SET status='SNAPSHOT_PENDING',failure_reason=NULL WHERE id=$1", [id]);
  } else if (!event.deployment_block) {
    type = 'DEPLOY_EVENT'; dedupeKey = `deploy:${id}`; message = 'Deployment retry queued';
  ...
  const job = await transaction((client) => enqueueJob({ eventId: id, type, dedupeKey, message, client }));
  kickJobRunner();
