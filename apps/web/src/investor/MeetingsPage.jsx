...
import { ArrowIcon, ErrorMessage, InvestorFrame, SecurityIdentity, StandingDisclosure } from './InvestorFrame.jsx';
...
export default function MeetingsPage() {
  ...
  const matching = groups[tab.id].filter((event) => [event.title, event.issuerName, event.securityName, event.securityTicker, event.tokenName, event.platform]
  ...
  return <InvestorFrame><section className="investor-meetings investor-page-width">
    ...
    <div className="investor-tabs" role="tablist" aria-label="Meeting categories">
      {MEETING_TABS.map((item) => <button type="button" key={item.id} role="tab" id={`tab-${item.id}`}
        ...
        }} onClick={() => { setParameters({ tab: item.id }); setSearch(''); }}>{item.label}</button>)}
    </div>
    <section id="meeting-tab-panel" role="tabpanel" aria-labelledby={`tab-${tab.id}`}>
      <label className="investor-search-label" htmlFor="meeting-search">{tab.search}</label>
      <div className="investor-search"><input id="meeting-search" type="search" value={search}
        onChange={(event) => setSearch(event.target.value)} /><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="10" cy="10" r="6" /><path d="m15 15 5 5" /></svg></div>
      ...
      {meetings.loading ? <p role="status">Loading your eligible meetings...</p> : !matching.length
        ? <p className="investor-empty">{search ? 'No meetings match your search.' : 'There are no meetings in this category.'}</p> : null}
      <div className="investor-meeting-list">{matching.map((event) => {
        ...
        return <article className="investor-meeting-row" key={event.id}>
          <div><div className="investor-meeting-title"><IssuerLogo event={event} className="investor-list-issuer-logo" /><h2>{event.title}</h2></div><SecurityIdentity event={event} />
            <div className="investor-tags">{event.platform && <span className="investor-tag">{event.platform}</span>}
              {scheduled && <span className="investor-tag muted">Scheduled</span>}
              {voted && <span className="investor-tag muted">{event.voteStatus === 'CONFIRMED' ? 'Voted' : 'Vote submitted'}</span>}</div>
